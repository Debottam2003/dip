...
def main():
    ...
    if not (0 <= salt_probability <= 1) or not (0 <= pepper_probability <= 1):
        print("Please enter valid probabilities between 0 and 1.")
        return
    noisy_image = add_salt_and_pepper_noise(original_image, salt_probability, pepper_probability)
    cv2.imshow('Original Image', original_image)
    cv2.imwrite('Noisy Image.jpg0', noisy_image)
    cv2.waitKey(0)
